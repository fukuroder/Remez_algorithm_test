...
# デバッグ用
def debug_plot(list_a, list_x, w0, h, err, filepath):
    def ef(x):
        return tri_polynomial(x, list_a)-ideal_lowpass_filter(x, w0)

    plt.plot( [0,w0,w0,math.pi], [1, 1, 0, 0], "b")
    plt.errorbar([w0, w0, w0, w0], [0.2, 0.4, 0.6, 0.8], xerr=h/2, fmt="b")

    if len(list_a) > 0:
        xx = numpy.linspace(0, math.pi,1024)
        yy = [tri_polynomial(x,list_a) for x in xx]
        plt.plot(xx,yy, "r")

    if err==1:
        yy =  [ ideal_lowpass_filter(x, w0) for x in list_x]
        yy2 = [ ef(x) for x in list_x]
        plt.errorbar(list_x, yy, yerr=yy2, fmt='go')
    else:
        yy =  [ideal_lowpass_filter(x, w0) for x in list_x]
        plt.plot(list_x, yy, 'go')
...
def remez(
    ...
    debug_plot([], list_x, w0, h, 0, "001_step1.png")
    ...
    for count in range(1, max_iter+1):
        ...
        debug_plot(list_a, list_x, w0, h, 1, str(count).zfill(3) + "_step2.png")

        print( "iteration:" + str(count) + "\t" + "ripple width:" + str(d))

        # Remez algorithm --step3--
        # 誤差関数の絶対値が最大となるn+2個の点を求める
        list_x = update_maximum_error_points(list_a, w0, h)

        debug_plot(list_a, list_x, w0, h, 0, str(count).zfill(3)  + "_step3.png")
